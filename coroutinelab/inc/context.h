...
#define EXPAND_CALL_7(args) EXPAND_CALL_6(args), (std::get<6>(args))//根据调用数量自动展开连接相应参数

#define CALLER_IMPL(func, x, args)                                             \
  if constexpr (std::tuple_size_v<std::decay_t<decltype(args)>> == x)          \
  func(EXPAND_CALL_##x(args))//当参数数量和args的size一样时调用
...
#define CALL(func, args)                                                       \
  ...
  CALLER_IMPL(func, 7, args);//根据参数数量调用不同的函数
